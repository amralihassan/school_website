				...
				<div class="panel-body">
					<div class="row">
						...
						<div class="col-lg-9">
							...
							<!-- button -->
							<button style="margin-top: -3px;" type="button" class="btn btn-info" onclick="searchData(1)">View</button>								
						</div>
					</div>
	        		<div class="col-lg-12">
						<div id="attandance_table"></div>
	        		</div>					
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	function searchData(page) 
	{
		...
		$.ajax({
			url:'<?php echo base_url() ?>Absence/pagination_search_parent/' + page,
			method:'get',
			data:{studentID:student_id.val()},
			dataType:'json',
			success:function(data){
				$('#attandance_table').html(data.attandance_table);
