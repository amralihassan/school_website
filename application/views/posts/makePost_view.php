...
<div class="box box-primary">
    <div class="box-body">
      <form method="POST" action="" id="new_post_form">
          ...
          <!-- select group -->
          <div class="row" style="display: none;margin-bottom: 5px;" id="private">
            <div class="col-md-4">
                <select multiple="multiple" class="form-control" name="groupName[]"  id="groupNameID">
                </select>
            </div>
          </div>
          <!-- post -->
...
<script type="text/javascript">
    ...
    function load_groups()
    {
      ...
      $.ajax({
          type:'ajax',
          url:'<?php echo base_url() ?>Notication/load_groups',
          dataType:'json',
    ...
    function publish()
    {
        var groups = $('select[name=groupName]').val();
        ...
        var file = document.getElementById('file_name').files[0]; //userfile file tag id
            ...
            {
                $('.alert-danger').html('Plese enter post to publish').fadeIn().delay(4000).fadeOut('slow');
                return;
            }
        $.ajax({
                type:'ajax',
